...
export const Background = ({
  ...
  const safeSource = src || "/placeholder.svg"
  const extension = getFileExtension(safeSource)
  const isVideoFile = isVideo(extension)

  const classNames = "absolute bg-background left-0 top-0 w-full h-full object-cover"

  if (isVideoFile) {
    return <VideoWithPlaceholder src={safeSource} className={classNames} placeholder={placeholder} />
  }
  ...
  return (
    <Image
      priority
      loading="eager"
      src={safeSource || "/placeholder.svg"}
      alt="Background"
